...
class searchresults extends Component {
    ...
    _loadProducts = () => {
        ...
        firebase.database().ref('products').on('value', (data) => {
                ...
                data.forEach((item) => {
                    ...
                    let hasMatch = false;
                    let matchScope = [pname, pbrand, pcate, psubcate, pdes];
                    matchScope.forEach((field) => {
                        if (this._matchKeyword(field)) {
                            hasMatch = true;
                        }
                    });

                    if (item != null && pid != null && pname != null && hasMatch) {
                        productList.push({
                            pid: pid,
                            pname: pname,
    ...
    _loadCategories = () => {
        firebase.database().ref('categories/beauty').on('value', (data) => {
                const cateList = [];
                ...
                <Row>
                    <Col xs={12} md={8} lg={3}>
                        <Breadcrumb tag="nav">
                            ...
                            <BreadcrumbItem active tag="a" href="/beauty">Results</BreadcrumbItem>
                        </Breadcrumb>
                    </Col>
                    <Col xs={12} md={8} lg={9}>
